...
class train_loader(object):
    def __init__(self, train_list, train_path, musan_path, rir_path, num_frames, **kwargs):
        ...
        # RIR
        # self.rir_files = []
        # for rir_file_name in os.listdir(rir_path):
        #   if rir_file_name.endswith(".wav"):
        #     self.rir_files.append(os.path.join(rir_path, rir_file_name))
    ...
    def __getitem__(self, index):
        ...
        # Returning audio data and label
        audio_tensor = torch.FloatTensor(audio[0])
        label = self.data_label[index]

        # Save audio as a sound file
        # soundfile.write(f'/augmented_data/audio_a_{index}.wav', audio[0], sr)
